...
import { getFullAudioPath, processAudioForTranscription } from "../utils/audioProcessor"
...
const ReportScreen: React.FC<ReportScreenProps> = ({ route, navigation }) => {
    ...
    const [isProcessing, setIsProcessing] = useState<boolean>(initialProcessingState)
    const [transcriptionLoading, setTranscriptionLoading] = useState<boolean>(initialProcessingState && !transcription)
        ...
        const setupAudio = async () => {
            try {
                ...
                try {
                    ...
                    if ((!audioUri || audioUri === "") && route.params.recordingTimestamp) {
                        ...
                        if (matchingFiles.length > 0) {
                            console.log("Found matching file by timestamp:", matchingFiles[0])
                    ...
                    if ((!audioUri || audioUri === "") && files.length > 0) {
                        ...
                        console.log("No URI provided, using most recent file:", sortedFiles[0])
                ...
                setPlaybackError(`Failed to set up audio: ${error.message || "Unknown error"}`)
    ...
    const setupSound = async (audioPath?: string) => {
        try {
            ...
            if (!soundUri || soundUri === "") {
                console.log("No valid audio URI provided, trying to find available audio files")
                ...
                if (availableFiles.length > 0) {
                    ...
                    console.log("Using most recent available audio file:", availableFiles[0])
            ...
            setPlaybackError(`Failed to set up audio playback: ${error.message || "Unknown error"}`)
            setIsProcessing(false)
        }
    }
    ...
    const setupSoundWithPath = async (soundUri: string) => {
        try {
            ...
            const possiblePaths = [
                ...
                FileSystem.documentDirectory ? `${FileSystem.documentDirectory}functions/audio/${filename}` : null,
            ...
            if (!foundAudioPath && route.params.audioUri && !route.params.skipCopyingAudio) {
                try {
            ...
            if (!foundAudioPath || !fileInfo || !fileInfo.exists) {
                setPlaybackError(`Could not find audio file. Tried paths: ${possiblePaths.join(", ")}`)
                setIsProcessing(false)
                return
            }
            ...
            if (initialStatus.isLoaded && typeof initialStatus.durationMillis === "number") {
                ...
                console.log(`Initial duration: ${initialStatus.durationMillis / 1000}s`)
            }
    ...
    const onPlaybackStatusUpdate = (status: any) => {
        ...
        console.log(
            "Status update:",
            status.isLoaded
                ? `playing: ${status.isPlaying}, pos: ${status.positionMillis / 1000}s, dur: ${
    ...
    const processAudio = async () => {
        try {
            ...
            if (route.params.transcription) {
                console.log("Already have transcription from params, skipping processing")
                setIsProcessing(false)
                return null
            }
            ...
            if (!audioUri || audioUri === "") {
                ...
                if (availableFiles.length > 0) {
                    ...
                    console.log("Using most recent audio file for processing:", mostRecentFile)
                    ...
                    if (fileInfo.exists) {
                        ...
                        if (route.params.transcription) {
                            ...
                            try {
                                ...
                                const result = await processAudioForTranscription(path, keepLocalFiles)
    ...
    const togglePlayback = async () => {
        ...
        try {
            ...
            setPlaybackError(`Failed to play audio: ${error.message || "Unknown error"}`)
        }
    }
        ...
        return (
            <View style={styles.audioPlayerContainer}>
                ...
                <View style={styles.progressContainer}>
                    <Text style={styles.timeText}>{formatTimeNoMilliseconds(position)}</Text>
                    ...
                    <Text style={styles.timeText}>{formatTimeNoMilliseconds(duration)}</Text>
                </View>
    ...
    const handleTextChange = (setText: React.Dispatch<React.SetStateAction<string>>, value: string) => {
        setText(value)
            ...
            <View style={styles.tabsContainer}>
                ...
                >
                    <FontAwesome
                        ...
                        style={styles.tabIcon}
                    />
                    <Text style={[styles.tabText, activeTab === "data" && styles.activeTabText]}>Report</Text>
                </TouchableOpacity>
            </View>
        )
    }
    ...
    const renderDataTab = () => {
        const recommendationsLoading = transcriptionLoading || (transcription && !recommendations)
        ...
        return (
            <View style={styles.tabContent}>
                {transcriptionLoading ? (
                    <View style={styles.formSection}>
                        <View style={styles.sectionHeader}>
                            <FontAwesome name="file-text-o" size={18} color="#007AFF" style={styles.sectionIcon} />
                            <Text style={styles.sectionHeaderText}>Transcription</Text>
                        </View>
                        <View style={styles.loadingContainer}>
                            <ActivityIndicator size="small" color="#007AFF" />
                            <Text style={styles.loadingText}>Generating transcription...</Text>
                        </View>
                    </View>
                ) : transcription ? (
                    <View style={styles.formSection}>
                        <View style={styles.sectionHeader}>
                            ...
                            <View style={styles.transcriptionActions}>
                                ...
                                >
                                    <Text style={styles.toggleText}>{showTranscription ? "Hide" : "Show"}</Text>
                                </TouchableOpacity>
                            </View>
                        </View>
                        {showTranscription && (
                ...
                {recommendationsLoading ? (
                    <View style={styles.formSection}>
                        <View style={styles.sectionHeader}>
                            <FontAwesome name="lightbulb-o" size={18} color="#F5A623" style={styles.sectionIcon} />
                            <Text style={styles.sectionHeaderText}>AI Recommendations</Text>
                        </View>
                        <View style={styles.loadingContainer}>
                            <ActivityIndicator size="small" color="#007AFF" />
                            <Text style={styles.loadingText}>Generating medical recommendations...</Text>
                        </View>
                    </View>
                ) : recommendations ? (
                    <View style={styles.formSection}>
                            ...
                            <TouchableOpacity
                                onPress={() => {
                                    if (recommendations) {
                                        ...
                                        Alert.alert("Copied", "Recommendations copied to clipboard")
                                    }
                                }}
                                style={styles.copyButton}
                            >
        ...
        try {
            ...
            try {
                ...
                if (route.params.recordingTimestamp) {
                    ...
                    const matchingFiles = audioFiles.filter((file) => file.includes(timestamp.toString()))
    ...
    const findAvailableAudioFiles = async (): Promise<string[]> => {
        try {
            ...
            const { getFirebaseSyncedAudioFiles } = require("../utils/audioProcessor")
        ...
        const findAudioPath = async (): Promise<string | null> => {
            ...
            try {
                ...
                if (timestamp) {
                    ...
                    const matchingFiles = files.filter((file) => file.includes(timestampStr))
